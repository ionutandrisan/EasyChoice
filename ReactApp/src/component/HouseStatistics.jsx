...
class HouseStatistics extends Component {
    ...
    constructor(props) {
        ...
        this.getHouseNameFromState = this.getHouseNameFromState.bind(this);
    }
    ...
    getNoClientsGraph(jwt) {
        PersonDataService.getNrOfClientsForHouse(jwt, this.state.id)
            .then((response) => {
                this.setState({
    ...
    getNumberOfReviews(jwt) {
        PersonDataService.getNrReviewsForHouse(jwt, this.state.id)
            .then((response) => {
                this.setState({
    ...
    getReviewRatingsForUser(jwt) {
        PersonDataService.getReviewRatingsForHouse(jwt, this.state.id)
            .then((response) => {
                this.setState({
    ...
    getHouseNameFromState() {
        for(let i = 0; i < this.state.housesName.length; i++) {
            if (this.state.housesName[i].houseID == this.state.id) {
                return this.state.housesName[i].houseName;
            }
        }
        return null;
    }
    ...
    render() {
        ...
        if (localStorage.getItem("jwt") && this.state.noClientsGraphData && this.state.nrReviews && this.state.reviewRatings) {
            ...
            const houseN = this.getHouseNameFromState();
            title = <div className="statisticsTitile">
                Hello, Mr/Ms {this.state.lastName}. Here is your summary for your house, <span style={{ fontWeight: "bold" }}>{houseN}</span>, for year 2020
            </div>
